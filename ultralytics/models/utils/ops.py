...
def get_cdn_group_pose(
    batch, num_classes, num_queries, class_embed, num_dn=100, cls_noise_ratio=0.5, box_noise_scale=1.0, kpts_noise_scale=0.5, training=False, visibility=False
):
    """
    Get contrastive denoising training group with positive and negative samples from ground truths.
    ...
    Returns:
        padding_cls (Optional[torch.Tensor]): The modified class embeddings for denoising.
        padding_bbox (Optional[torch.Tensor]): The modified bounding boxes for denoising.
        padding_kpts (Optional[torch.Tensor]): The modified keypoints for denoising.
        attn_mask (Optional[torch.Tensor]): The attention mask for denoising.
        dn_meta (Optional[Dict]): Meta information for denoising.
    """
    if (not training) or num_dn <= 0 or batch is None:
    ...
    gt_cls = batch["cls"]  # (bs*num, )
    gt_bbox = batch["bboxes"]  # bs*num, 4
    gt_kpts = batch["kpts"] # (bs*num, )
    b_idx = batch["batch_idx"]
    # If visibility is True, we assume the kpts are in the format [17,3]
    if visibility:
        gt_kpts =  torch.cat([gt_kpts[:,i] for i in range(gt_kpts.size(0)) if (i+1) % 3 != 0])
                
    # Each group has positive and negative queries.
    dn_cls = gt_cls.repeat(2 * num_group)  # (2*num_group*bs*num, )
    dn_bbox = gt_bbox.repeat(2 * num_group, 1)  # 2*num_group*bs*num, 4
    dn_kpts = gt_kpts.repeat(2 * num_group, 1) #  (2*num_group*bs*num, )
    dn_b_idx = b_idx.repeat(2 * num_group).view(-1)  # (2*num_group*bs*num, )
    ...
    if kpts_noise_scale > 0:
        # kpts depend on bbox
        diff = (dn_bbox[..., 2:] * 0.5) * kpts_noise_scale  # 2*num_group*bs*num, 2
        
        if visibility:
            for i in range(2,diff.shape[-1],3):
                diff[:,i] = 0.0

        rand_sign = torch.randint_like(dn_kpts, 0, 2) * 2.0 - 1.0
        rand_part = torch.rand_like(dn_kpts)
        rand_part[neg_idx] += 1.0
        
        kpts_offset = rand_part * rand_sign 
        
        for i in range(kpts_offset.shape[-1]):
            kpts_offset[:,i] *= diff[:,i%2]
        
        dn_kpts += kpts_offset
        dn_kpts.clip_(min=0.0, max=1.0)
        dn_kpts = torch.logit(dn_kpts,eps=1e-6)
    ...
    padding_cls = torch.zeros(bs, num_dn, dn_cls_embed.shape[-1], device=gt_cls.device)
    padding_bbox = torch.zeros(bs, num_dn, 4, device=gt_bbox.device)
    padding_kpts = torch.zeros(bs, num_dn, dn_kpts.shape[-1], device=gt_kpts.device)
    ...
    padding_cls[(dn_b_idx, map_indices)] = dn_cls_embed
    padding_bbox[(dn_b_idx, map_indices)] = dn_bbox
    padding_kpts[(dn_b_idx, map_indices)] = dn_kpts
    ...
    return (
        padding_cls.to(class_embed.device),
        padding_bbox.to(class_embed.device),
        padding_kpts.to(class_embed.device),
        attn_mask.to(class_embed.device),
        dn_meta,
    )
