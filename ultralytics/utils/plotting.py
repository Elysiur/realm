    ...
    if isinstance(x, list):
        for idx, y in enumerate(x):
            ...
            if height > 1 and width > 1:
                ...
                blocks = torch.chunk(y[0].cpu(), channels, dim=0)  # select batch index 0, block by channels
                n = min(n, channels)  # number of plots
                _, ax = plt.subplots(math.ceil(n / 8), 8, tight_layout=True)  # 8 rows y n/8 cols
                ax = ax.ravel()
                ...
                plt.close()
                np.save(str(f.with_suffix(".npy")), y[0].cpu().numpy())  # npy save
